...
### 6. Skipping Tests Instead of Fixing CI Dependencies

When the E2E integration test failed in CI due to missing `azure-storage-blob`, the initial response was to add `pytest.mark.skipif` decorators. This is **wrong** - a skipped test validates nothing.

**Root Cause:** Reflex to "make CI green" rather than "make tests actually run."

**Correct Approach:** Add missing dependencies to CI workflow. If a test needs a package to import, that package must be in CI requirements.

**Rule:** Never use skip decorators for import/dependency errors. Always fix CI.
...
- **NEVER skip tests for import/dependency errors** - Fix CI dependencies instead. A skipped test is a test that doesn't validate anything. If a test fails due to missing imports, add the dependency to CI - don't add skip decorators.
