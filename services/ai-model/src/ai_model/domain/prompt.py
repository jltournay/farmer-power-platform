"""Prompt domain model for AI Model service.

This module defines the Pydantic models for prompt storage with versioning.
Prompts are stored in the ai_model.prompts MongoDB collection.

Key design decisions (see architecture/ai-model-architecture/key-decisions.md):
- Single collection with prompt_type discriminator field
- All 5 agent types share one collection
- Version + status lifecycle for prompt management
"""
...
class PromptContent(BaseModel):
    ...
    system_prompt: str = Field(description="Full system prompt text that sets agent behavior")
...
class Prompt(BaseModel):
    ...
    prompt_id: str = Field(description="Logical prompt identifier (e.g., 'disease-diagnosis')")
    agent_id: str = Field(description="Links to agent config (e.g., 'diagnose-quality-issue')")
