...
This module provides a generic base repository pattern for MongoDB persistence.
Following the pattern from plantation-model for consistency.
"""
...
class BaseRepository(Generic[T]):
    ...
    async def update(self, entity_id: str, updates: dict) -> T | None:
        ...
        """
        # Add updated_at timestamp if the model has metadata
        if "metadata" in updates and isinstance(updates["metadata"], dict):
            updates["metadata"]["updated_at"] = datetime.now(UTC)
        elif "metadata.updated_at" not in updates:
            updates["metadata.updated_at"] = datetime.now(UTC)
