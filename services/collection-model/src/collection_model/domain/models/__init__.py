...
class DocumentStoredEvent(BaseModel):
    ...
    source_type: str = Field(
        description="Type of source: 'qc_analyzer_result', 'qc_analyzer_exception', etc."
    )
    farmer_id: str | None = Field(
        default=None, description="Farmer ID if identifiable from document"
    )
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When the document was stored"
    )
    event_type: str = Field(
        default="document.stored", description="Event type identifier"
    )
...
class PoorQualityDetectedEvent(BaseModel):
    ...
    primary_percentage: float = Field(
        description="Current primary grade percentage (0-100)"
    )
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When quality issue detected"
    )
    event_type: str = Field(
        default="poor_quality_detected", description="Event type identifier"
    )
...
class WeatherUpdatedEvent(BaseModel):
    ...
    temperature_high: float | None = Field(
        default=None, description="High temperature in Celsius"
    )
    temperature_low: float | None = Field(
        default=None, description="Low temperature in Celsius"
    )
    rainfall_mm: float | None = Field(default=None, description="Rainfall in mm")
    humidity_percent: float | None = Field(
        default=None, description="Average humidity percentage"
    )
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When weather was updated"
    )
    event_type: str = Field(
        default="weather.updated", description="Event type identifier"
    )
...
class MarketPricesUpdatedEvent(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="When price was updated"
    )
    event_type: str = Field(
        default="market_prices.updated", description="Event type identifier"
    )
