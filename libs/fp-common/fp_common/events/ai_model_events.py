"""AI Model event payload models for DAPR pub/sub.

Story 0.75.16b: Event Subscriber Workflow Wiring

This module defines Pydantic models for AI Model events that need to be
shared across services (AI Model, Collection Model, etc.):
...
These models are published by AI Model and consumed by domain models
(Collection, Plantation, etc.) to trigger AI processing and receive results.
"""
...
class AgentRequestEvent(BaseModel):
    ...
    source_service: str | None = Field(
        default=None,
        description="Optional: explicit source service name for observability "
        "(e.g., 'collection-model', 'farm-model'). Defaults to source if not provided.",
    )
